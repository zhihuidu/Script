...
do
	...
	do
                TmpStr=${j##*$i-}
                P0Frequency=${TmpStr%.*}
                P0AvgFre=$(echo $P0Frequency | tr '-' '\n' |awk 'BEGIN {a=0} {a+=$1} END { print a/NR/1000 }')
		P0MaxFre=$(echo $P0Frequency | tr '-' '\n' |awk 'BEGIN {max = 0} {if ($1>(max+0.0)) max=$1 fi} END {print max/1000}') 
		P0MinFre=$(echo $P0Frequency | tr '-' '\n' |awk 'BEGIN {min = 1999} {if ($1<(min+0.0)) min=$1 fi} END {print min/1000}')
		...
		P0FreDis=$(echo $P0Frequency | tr '-' ' '|awk '{print $1/1000 " " $2/1000  " " $3/1000" " $4/1000 }')
...
# 1:Power, 2:Time , 3:Energy, 4:AvgFre, 5:DiffMaxAvg, 6:DiffMaxMin,7:DiffAvgMin,8:VarianceFre, 9:SDFre, 10:MaxFre,11:MinFre, 12:FreDis 
