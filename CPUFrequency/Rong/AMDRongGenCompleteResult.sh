...
do
	echo "Checking $i"
	if  exists_pattern_files $i/*$i*.power.machine ; then 
	cd $i
	echo "enter directory $i"
	rm -f $i.n0*avg.txt
	for node in n03 n04 n05
	do
	...
	do
		...
		FreDis=$(echo $Frequency | tr '-' ' '|awk '{print $1/10 " " $2/10  " " $3/10" " $4/10 }')
...
# 1:Power, 2:Time , 3:Energy, 4:AvgFre, 5:DiffMaxAvg, 6:DiffMaxAvg, 7:DiffMaxMin,8:DiffAvgMin,9:VarianceFre, 10:SDFre, 11:MaxFre,12:MinFre, 13:FreDis >>$i.${node}avg.txt
		echo  $Power " " $Time " " $Energy " " $AvgFre " " $DiffMaxAvg " " $DiffMaxAvg " " $DiffMaxMin " " $DiffAvgMin " " $VarianceFre " " $SDFre " " $MaxFre " " $MinFre " " $FreDis >>$i.${node}avg.txt
