...
#            [936364282,39459925,2,0,HomeDir+"Adata/SNAP/uk-2005.mtx"],\
    ...
    with open(output_file, "w", newline="") as csvfile:
        ...
        csv_writer.writerow(["Graph Name","Graph ID","Number of Edges", "Number of Vertices"])
        ID=0
        for mylist in [EdgeList,MtxOther, BigMtxFile,MtxDelaunay]:
            for item in mylist:
                filename = pathlib.Path(item[4])
                ...
                shortname = filename
                print("The output filename:", shortname)
                csv_writer.writerow([shortname,ID,item[0], item[1]])
                ID=ID+1
    with open('myoutput.csv', 'r') as infile:
            reader = csv.reader(infile)
            data = list(reader)
            transposed_data = list(map(list, zip(*data)))
            with open('output_transposed.csv', 'w', newline='') as outfile:
